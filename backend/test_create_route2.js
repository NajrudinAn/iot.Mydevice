...
(async () => {
    try {
        ...
        const { rows: dRows } = await pool.query('SELECT id FROM devices WHERE workspace_id = $1 LIMIT 1', [wsId]);
        const devId = dRows[0]?.id;
        console.log("WS", wsId, "USER", userId, "DEV", devId);
